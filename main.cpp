...
Nodo* retornarArbol(stack<int> camino, int n){  // implementación del árbol

    if (!camino.empty()){
        Nodo* raiz = new Nodo(numeroALetra(camino.top(),n));
        Nodo* aux = raiz;
        camino.pop();

    do {

        aux->sig = new Nodo(numeroALetra(camino.top(), n)); 
        aux = aux->sig; 
        camino.pop();
...
Nodo* dijkstra(vector<vector<int>> matriz, int nodo, int objetivo, int &dist) {
    int n = matriz.size();
        ...
        if (fila != -1) {
            ...
            if (!visitados[i] && matriz[fila][i]!= 0 && caminos[fila]+matriz[fila][i] < caminos[i]) {
                ...
                if (fila == objetivo) {
                    ...
                    return retornarArbol(invertir,n); // cambio de retorno para imprimir el camino
                }
        }
    }
...
int main(){
    ...
    int dist;
    Nodo* menorCamino = dijkstra(matriz,0, objetivo,dist);
    Nodo* menorCaminoaux = menorCamino;
    cout << "El menor camino para " <<numeroALetra(objetivo, n);
    if (menorCamino != nullptr){
        cout <<" es: \n" << endl;


        while (menorCaminoaux ->sig != nullptr)    // cambio de recorrido para imprimir el camino
    {
        
        cout << menorCaminoaux ->letra << " -> " ;
        menorCaminoaux = menorCaminoaux -> sig;
